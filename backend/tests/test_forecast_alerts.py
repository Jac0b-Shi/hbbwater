...
class ForecastAlertTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_legacy_scenario_pump_count_one_or_three_is_rejected(self):
        """1 or 3 pumps are not supported until q1/q3 are calibrated."""
        from app.schemas import ForecastAlertProfileResponse

        for invalid_count in (1, 3):
            with self.assertRaises(ValueError):
                ForecastAlertProfileResponse(
                    sensor_id="ultrasonic_002",
                    is_enabled=True,
                    pump_params={
